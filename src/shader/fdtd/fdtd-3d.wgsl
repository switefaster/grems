...
@group(0)
@binding(2)
var constants_map: texture_storage_3d<rgba32float, read>;

@group(1)
@binding(0)
var additive_source: texture_storage_3d<rgba32float, read>;
...
fn update_magnetic_field(@builtin(global_invocation_id) global_invocation_id: vec3<u32>) {
    if (global_invocation_id.x < c_param.dimension.x && global_invocation_id.y < c_param.dimension.y && global_invocation_id.z < c_param.dimension.z) {
        ...
        let consts = textureLoad(constants_map, texel).xyz;
        let prev_h = textureLoad(update_field, texel).xyz * consts.x + textureLoad(additive_source, texel).xyz * consts.z;
        ...
        var store_value = prev_h + consts.y * vec3<f32>(diff_hx, diff_hy, diff_hz);
        if (texel.x == i32(c_param.dimension.x) - 1) {
            store_value.y = 0.0;
            store_value.z = 0.0;
        }
...
fn update_electric_field(@builtin(global_invocation_id) global_invocation_id: vec3<u32>) {
    if (global_invocation_id.x < c_param.dimension.x && global_invocation_id.y < c_param.dimension.y && global_invocation_id.z < c_param.dimension.z) {
        let texel = vec3<i32>(global_invocation_id);
        let consts = textureLoad(constants_map, texel).xyz;
        let prev_e = textureLoad(update_field, texel).xyz * consts.x + textureLoad(additive_source, texel).xyz * consts.z;
        ...
        var store_value = prev_e + consts.y * vec3<f32>(diff_ex, diff_ey, diff_ez);
        if (texel.x == i32(c_param.dimension.x) - 1) {
            store_value.x = 0.0;
        }
        if (texel.y == i32(c_param.dimension.y) - 1) {
