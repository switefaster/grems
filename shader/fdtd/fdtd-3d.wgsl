...
fn update_magnetic_field(@builtin(global_invocation_id) global_invocation_id: vec3<u32>) {
    if (global_invocation_id.x < c_param.dimension.x && global_invocation_id.y < c_param.dimension.y && global_invocation_id.z < c_param.dimension.z) {
        ...
        if (texel.x < i32(c_param.dimension.x) - 1) {
            ...
            e_shift_x = vec3<f32>(textureLoad(conjugative_field_x, actual_texel).x, textureLoad(conjugative_field_y, actual_texel).x, textureLoad(conjugative_field_z, actual_texel).x);
        }
        var e_shift_y = vec3<f32>(0.0);
        if (texel.y < i32(c_param.dimension.y) - 1) {
            let actual_texel = vec3<i32>(texel.x, texel.y + 1, texel.z);
            e_shift_y = vec3<f32>(textureLoad(conjugative_field_x, actual_texel).x, textureLoad(conjugative_field_y, actual_texel).x, textureLoad(conjugative_field_z, actual_texel).x);
        }
        var e_shift_z = vec3<f32>(0.0);
        if (texel.z < i32(c_param.dimension.z) - 1) {
            let actual_texel = vec3<i32>(texel.x, texel.y, texel.z + 1);
            e_shift_z = vec3<f32>(textureLoad(conjugative_field_x, actual_texel).x, textureLoad(conjugative_field_y, actual_texel).x, textureLoad(conjugative_field_z, actual_texel).x);
        }
...
fn update_electric_field(@builtin(global_invocation_id) global_invocation_id: vec3<u32>) {
    if (global_invocation_id.x < c_param.dimension.x && global_invocation_id.y < c_param.dimension.y && global_invocation_id.z < c_param.dimension.z) {
        ...
        if (texel.x > 0) {
            let actual_texel = vec3<i32>(texel.x - 1, texel.y, texel.z);
            h_shift_x = vec3<f32>(textureLoad(conjugative_field_x, actual_texel).x, textureLoad(conjugative_field_y, actual_texel).x, textureLoad(conjugative_field_z, actual_texel).x);
        }
        var h_shift_y = vec3<f32>(0.0);
        if (texel.y > 0) {
            let actual_texel = vec3<i32>(texel.x, texel.y - 1, texel.z);
            h_shift_y = vec3<f32>(textureLoad(conjugative_field_x, actual_texel).x, textureLoad(conjugative_field_y, actual_texel).x, textureLoad(conjugative_field_z, actual_texel).x);
        }
        var h_shift_z = vec3<f32>(0.0);
        if (texel.z > 0) {
            let actual_texel = vec3<i32>(texel.x, texel.y, texel.z - 1);
            h_shift_z = vec3<f32>(textureLoad(conjugative_field_x, actual_texel).x, textureLoad(conjugative_field_y, actual_texel).x, textureLoad(conjugative_field_z, actual_texel).x);
        }
